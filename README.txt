...
* 30 business-focused analytical SQL queries
...
```text
CUSTOMERS ──┬──< ORDERS1 ──< ORDER_ITEMS1 >── PRODUCTS1
             │
             └──< CUSTOMER_ACTIVITY1
```
...
| Table                | Grain                        | Purpose                                                          |
| -------------------- | ---------------------------- | ---------------------------------------------------------------- |
| `CUSTOMERS`         | One row per customer         | Firmographics, region, industry, annual revenue, account manager |
| `PRODUCTS`          | One row per product          | 20 SaaS products across multiple business categories             |
| `ORDERS`            | One row per order            | Order header, order date, customer, sales channel                |
| `ORDER_ITEMS`       | One row per order line       | Product quantity, pricing, discount, revenue calculation         |
| `CUSTOMER_ACTIVITY` | One row per monthly activity | Logins, support tickets, satisfaction score, engagement metrics  |
